...
class User {
  constructor(name, money, funcion) {
    this.name = name;
    this.money = money;
    this.showMeTheMoney = funcion;
   
  }
}


// Funcion showMeTheMoneyFunc
var valorFuncion = function showMeTheMoneyFunc () {
  alert("dinero del usuario");
}
...
.then(data => {
  ...
  //Recorremos con un for todos los objetos
  for (let i = 0; i <data.length; i++) {
    let valor = new User(data[i].name, data[i].money, valorFuncion);
    //Añadimos cada objeto al array users
    users.push(valor);
  }

//Mostramos array por consola y llamamos a la funcion.
console.log(users)
users[3].showMeTheMoney();
